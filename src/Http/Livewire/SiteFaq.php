 ...
 */
class SiteFaq extends Component
{
    public $actions = [];
    public $_id;
    ...
    public function mount()
    {
        // 뷰이미지
        if(!$this->viewFile) {
            $this->viewFile = "jiny-site-cms::site.faq.dashboard.cate";
        }
    ...
    {
        $cates = DB::table('site_faq_cate')->get();
    ...
    public function render()
    {
        switch($this->mode) {
            case 'search':
                $rows = DB::table('site_faq')
                    ->where('title','like', "%".$this->search_keyword."%")
                    ->get();
                return view("jiny-site-cms::site.faq.search",[
                    'rows' => $rows
                  ]);
                break;
            case 'list':
                $rows = DB::table('site_faq')
                    ...
                    ->get();
                return view("jiny-site-cms::site.faq.category.list",[
                  'rows' => $rows
                ]);
                break;
            ...
            case 'article':
                if(is_numeric($this->_id)) {
                    $article = DB::table('site_faq')->where('id',$this->_id)->first();
                } else {
                    $article = DB::table('site_faq')->where('slug',$this->_id)->first();
                }
                if($article) {
                    return view("jiny-site-cms::site.faq.article.detail",[
                        'article' => $article
                    ]);
                }

                return view("jiny-site-cms::site.faq.article.error",[
    ...
    public function storeCategory()
    {
        ...
        DB::table('site_faq_cate')->insert($this->forms);
    ...
    {
        $row = DB::table('site_faq_cate')->where('id',$id)->first();
    ...
    public function storeArticle()
    {
        ...
        DB::table('site_faq')->insert($this->forms);
    ...
    {
        $row = DB::table('site_faq')->where('id',$id)->first();
    ...
    public function likeUp($id)
    {
        ...
        DB::table('site_faq')->where('id', $id)->increment('like');
    }

    public function likeDown($id)
    {
        // 해당 레코드의 like 값을 +1 증가
        DB::table('site_faq')->where('id', $id)->decrement('like');
    }
